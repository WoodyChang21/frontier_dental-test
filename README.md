...
**Category Subgraph** (runs inside `crawl_category`) — `extract_product` is where the Product Subgraph runs (one instance per URL, all in parallel via `Send()`). Note: no `batch_fetch_pages` node here — pages are fetched individually inside the Product Subgraph.

![Category Subgraph](assets/graph_category.png)

**Product Subgraph** (runs inside `extract_product`) — dashed edge from `extract_structured` to `llm_fallback` is the conditional last-resort path (no Algolia data AND CSS found no description).
